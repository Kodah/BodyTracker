...
class CompareOverlayViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let tabBar = tabBarController as? CompareTabViewController,
            let progressPointToCompare = tabBar.progressPointsToCompare {
            ...
            shareImage = UIImage.createCompareImage(progressPointToCompare.firstProgressPoint,
                                                    progressPoint2: progressPointToCompare.secondProgressPoint,
                                                    statsEnabled: true)

        }

        let leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.cancel,
                                                target: self,
                                                action: #selector(CompareOverlayViewController.dismissSelf))

        navigationItem.leftBarButtonItem = leftBarButtonItem

        let rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.save,
                                                 target: self,
                                                 action: #selector(
                                                    CompareOverlayViewController.showShareImageViewController))
